...
{
    private static List<Game> Load()
    {
        List<Game> videoGame = new List<Game>();

        if (!File.Exists("videogames.xml"))
        {
            return videoGame;
        }
        ...
        try
        {
            ...
            XmlSerializer formatter = new XmlSerializer(videoGame.GetType());
            Stream stream = new FileStream("videogames.xml",
                FileMode.Open, FileAccess.Read, FileShare.Read);
            videoGame = (List<Game>)formatter.Deserialize(stream);
            stream.Close();
            return videoGame;
    ...
    private static void Save(List<Game> videoGame)
    {
        try
        {
            ...
            formatter.Serialize(stream, videoGame);
            stream.Close();
        }
        catch (IOException)
        {
            Console.WriteLine("Reading error save");
        }
        catch (Exception e)
        {
            Console.WriteLine("Error: " + e.Message);
