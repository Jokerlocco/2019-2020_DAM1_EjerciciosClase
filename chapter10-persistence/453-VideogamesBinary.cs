...
class VideogamesManagementSystemVersion3
{
    static void Main()
    {
        List<Game> videoGame = new List<Game>();
        if (File.Exists("videojuegos.bin"))
        {
            BinaryReader input= new BinaryReader(
                 File.Open("videojuegos.bin", FileMode.Open));
            int tamanyo = input.ReadInt32();
            for (int i = 0; i < tamanyo; i++)
            {
                Game vg = new Game();
                vg.Title = input.ReadString();
                vg.Category = input.ReadString();
                vg.Platform = input.ReadString();
                vg.Year = input.ReadUInt16();
                vg.Rating = input.ReadString();
                vg.Comments = input.ReadString();
                videoGame.Add(vg);
            }
            input.Close();
        ...
        do
        {
            ...
            switch (option)
            {
                ...
                case "8":
                    for (int i = 0; i < videoGame.Count; i++)
                    {
                        videoGame[i].Title.Trim();
                        videoGame[i].Category.Trim();
                        videoGame[i].Platform.Trim();

                        while (videoGame[i].Title.Contains("  "))
                            videoGame[i].Title = 
                            videoGame[i].Title.Replace("  "," ");
                         while (videoGame[i].Category.Contains("  "))
                            videoGame[i].Category = 
                            videoGame[i].Category.Replace("  "," ");
                         while (videoGame[i].Platform.Contains("  "))
                            videoGame[i].Platform = 
                ...
                case "Q":
                    {
                        BinaryWriter output = new BinaryWriter(
                            File.Open("videojuegos.bin", FileMode.Create));
                        output.Write(videoGame.Count);
                        for (int i = 0; i < videoGame.Count; i++)
                        {
                            output.Write(videoGame[i].Title);
                            output.Write(videoGame[i].Category );
                            output.Write(videoGame[i].Platform);
                            output.Write(videoGame[i].Year);
                            output.Write(videoGame[i].Rating);
                            output.Write(videoGame[i].Comments);
                        }
                        output.Close();
                    }
