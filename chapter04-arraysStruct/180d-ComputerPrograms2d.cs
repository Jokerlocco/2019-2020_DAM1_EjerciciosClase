        ...
        do
        {
            ...
            switch (option)
            {
                ...
                case '3':
                    ...
                    for (int i = 0; i < amountOfData; i++)
                    {
                        if (programs[i].name.ToUpper().Contains(search)
                            || programs[i].description.ToUpper().Contains(search)
                            || programs[i].category.ToUpper().Contains(search))
                        {
                            Console.WriteLine("Name: " + programs[i].name);
                            if (programs[i].category == "")
                            {
                                Console.WriteLine("Category: No data");
                            }
                            else
                            {
                            ...
                            Console.WriteLine
                                ("Version: "
                                + programs[i].version.releaseNumber + ": "
                ...
                case '4':
                    ...
                    {
                        Console.Write("Enter name (it was {0}): ",
                            programs[modifyPosition].name);
                        string newText = Console.ReadLine();
                        if (newText != "")
                            programs[modifyPosition].name = newText;

                        Console.Write("Enter category (it was {0}): ",
                            programs[modifyPosition].category);
                        newText = Console.ReadLine();
                        if (newText != "")
                            programs[modifyPosition].category = newText;

                        Console.Write("Enter description (it was {0}: ",
                            programs[modifyPosition].description);
                        newText = Console.ReadLine();
                        if (newText != "")
                            programs[modifyPosition].description = newText;
                        ...
                        newText = Console.ReadLine();
                        if (newText != "")
                            programs[modifyPosition].version.releaseNumber 
                                = newText;
                        ...
                        if (newText != "")
                            ...
                            Convert.ToByte(newText);

                        Console.Write("Year: ");
                        newText = Console.ReadLine();
                        if (newText != "")
                            programs[modifyPosition].version.year =
                            Convert.ToUInt16(newText);
                    }
                    break;
                ...
                case '7':
                    for (int i =0; i < amountOfData; i++)
                    {
                        programs[i].name = programs[i].name.Trim();
                        while (programs[i].name.Contains("  "))
                            programs[i].name = programs[i].name.Replace("  ", " ");
                    }
                    break;
