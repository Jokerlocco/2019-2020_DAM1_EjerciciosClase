...
{
    struct datoContable : IComparable< datoContable >
    {
        public string fecha;
        ...
        public int CompareTo( datoContable otro )
        {
            //return (int) (importe - ((datoContable) otro).importe);
            int comparaCFechas = fecha.CompareTo(otro.fecha);
            if (comparaCFechas != 0)
                return comparaCFechas;

            int comparacDescripc = descripcion.CompareTo(otro.descripcion);
            return comparacDescripc;
        }
    }
    ...
    {
        const int MAX = 10000;
        datoContable[] c = new datoContable[MAX];
        char opcion;
        int cantidad = 0;
